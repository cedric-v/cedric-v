...
4. Un lien signé, valable 7 jours, est envoyé par Mailjet. Chaque inscription en attente est enregistrée dans la KV (`cedricv:newsletter:pending:<email>`, TTL 8 jours —
préfixe site pour cohabiter avec le projet candc-ch dans le même namespace).
5. **Relance unique** : si la confirmation n’arrive pas sous ~20 h, le Worker cron `cedric-v-newsletter-remind` (Cloudflare Cron Trigger, 6 h 45 UTC) appelle `/api/newsletter-remind` (protégé par `NEWSLETTER_REMIND_KEY`) qui renvoie le même lien de confirmation ; la clé est purgée après envoi, ou à 7 jours (lien expiré) sans envoi. La confirmation supprime la clé. À chaque nouvel opt-in réussi, une notification est envoyée à `CONTACT_TO_EMAIL` (`optinNotification` dans `functions/utils/admin-notification.js`) ; un échec de notification n’interrompt jamais l’inscription.
6. La confirmation ajoute le contact à la liste, passe le statut à `confirme` et envoie le premier message de bienvenue, contenant deux CTA de segmentation (`Je suis indépendant / entrepreneur` → `/accompagnement/individuel/`, `Je dirige une TPE / PME` → `/digital-manager/`). Le clic passe par `/api/newsletter-segment` (token signé 90 j) qui enregistre les propriétés Mailjet `segmentation_profil` (`independant` | `tpe_pme`, auto-déclarée) et `date_segmentation` avant de rediriger vers la page de vente. Le message de bienvenue contient aussi un lien de désinscription signé (valable 90 jours) pointant vers `/api/newsletter-unsubscribe`. La désinscription passe `IsUnsubscribed=true` dans Mailjet (trace conservée) et enregistre `date_retrait_consentement` sur le contact.
7. Après confirmation, l’utilisateur est redirigé vers `/newsletter/confirmation/` (FR) ou `/en/newsletter/confirmed/` (EN).

Tout le parcours est bilingue : la `locale` (`fr` | `en`) est captée par le formulaire (`document.documentElement.lang`), porteuse du token signé et stockée sur le contact (`langue_source`), puis reprise par chaque e-mail (confirmation, relance, bienvenue, désinscription) et chaque redirection (`/en/newsletter/confirmed/`, `/en/accompagnement/individuel/`, `/en/digital-manager/`).
