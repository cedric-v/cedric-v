---
layout: base.njk
title: Monthly Meeting - Clarity - Fluance Pro
description: "Monthly online meeting to regain clarity in your professional activity and your work-life balance."
locale: en
permalink: /en/rdv/clarte/
translation: /rdv/clarte/
---

<section class="max-w-6xl mx-auto px-6 md:px-12 py-16 space-y-12">
  <header class="text-center space-y-6">
    <h1 class="text-4xl md:text-5xl font-semibold text-[#0A6BCE]">
      Clarity Meeting
    </h1>
    <p class="text-xl text-[#0f172a]/80">
      to move forward on YOUR current challenges
    </p>
  </header>

  <section class="section-card p-8 bg-white space-y-6">
    <h2 class="text-2xl font-semibold text-[#0A6BCE] text-center">A monthly online meeting</h2>
    
    <div class="grid md:grid-cols-2 gap-8">
      <div class="space-y-4">
        <h3 class="text-xl font-semibold text-[#0A6BCE]">Format</h3>
        <ul class="space-y-2 text-[#0f172a]/80">
          <li>• <strong>1 session of 60 minutes</strong> per month</li>
          <li>• Via <strong>videoconference</strong></li>
          <li>• Group of <strong>5 people maximum</strong></li>
          <li>• <strong>Question-answer format</strong> adapted to your current needs</li>
          <li>• <strong>Access to replay</strong> if you cannot attend</li>
        </ul>
      </div>
      
      <div class="space-y-4">
        <h3 class="text-xl font-semibold text-[#0A6BCE]">Content</h3>
        <ul class="space-y-2 text-[#0f172a]/80">
          <li>• <strong>Experience sharing</strong> among entrepreneurs</li>
          <li>• <strong>Practical exercises</strong> to clarify your vision</li>
          <li>• <strong>Personalized advice</strong> from Cédric</li>
          <li>• <strong>Action plan</strong> for the following month</li>
        </ul>
      </div>
    </div>
  </section>

  <section class="section-card p-8 bg-white space-y-6">
    <h2 class="text-2xl font-semibold text-[#0A6BCE] text-center">For whom?</h2>
    
    <div class="space-y-4">
      <p class="text-lg text-[#0f172a]/80">
        This meeting is designed for <strong>entrepreneurs and freelancers</strong> who want to:
      </p>
      
      <ul class="space-y-3 text-[#0f172a]/80">
        <li class="flex items-start gap-3">
          <span class="text-[#0A6BCE] font-bold text-xl">✓</span>
          <span>Regain <strong>clarity</strong> in their professional activity</span>
        </li>
        <li class="flex items-start gap-3">
          <span class="text-[#0A6BCE] font-bold text-xl">✓</span>
          <span>Better <strong>balance professional and personal life</strong></span>
        </li>
        <li class="flex items-start gap-3">
          <span class="text-[#0A6BCE] font-bold text-xl">✓</span>
          <span>Exchange with other entrepreneurs in a <strong>supportive atmosphere</strong></span>
        </li>
        <li class="flex items-start gap-3">
          <span class="text-[#0A6BCE] font-bold text-xl">✓</span>
          <span>Benefit from <strong>practical advice</strong> and <strong>regular support</strong></span>
        </li>
        <li class="flex items-start gap-3">
          <span class="text-[#0A6BCE] font-bold text-xl">✓</span>
          <span>Implement <strong>concrete actions</strong> to evolve their activity</span>
        </li>
      </ul>
    </div>
  </section>

  <section class="section-card p-8 bg-white space-y-6">
    <h2 class="text-2xl font-semibold text-[#0A6BCE] text-center">Topics covered</h2>
    
    <div class="grid md:grid-cols-2 gap-6">
      <div class="border-l-4 border-[#0A6BCE] pl-4 space-y-2">
        <h3 class="text-lg font-semibold text-[#0A6BCE]">Strategic clarity</h3>
        <ul class="text-[#0f172a]/80 text-sm space-y-1">
          <li>• Clarify your vision</li>
          <li>• Identify your priorities</li>
          <li>• Structure your activity</li>
          <li>• Define your objectives</li>
        </ul>
      </div>
      
      <div class="border-l-4 border-[#ffce2d] pl-4 space-y-2">
        <h3 class="text-lg font-semibold text-[#0A6BCE]">Work-life balance</h3>
        <ul class="text-[#0f172a]/80 text-sm space-y-1">
          <li>• Manage your time efficiently</li>
          <li>• Set healthy boundaries</li>
          <li>• Reduce stress</li>
          <li>• Rediscover pleasure</li>
        </ul>
      </div>
      
      <div class="border-l-4 border-[#0A6BCE] pl-4 space-y-2">
        <h3 class="text-lg font-semibold text-[#0A6BCE]">Activity development</h3>
        <ul class="text-[#0f172a]/80 text-sm space-y-1">
          <li>• Improve your offer</li>
          <li>• Attract more clients</li>
          <li>• Increase your profitability</li>
          <li>• Simplify your processes</li>
        </ul>
      </div>
      
    </div>
  </section>

  <section class="section-card p-8 bg-[#0A6BCE]/5 border-l-4 border-[#0A6BCE] rounded-r-lg space-y-6">
    <h2 class="text-2xl font-semibold text-[#0A6BCE]">How does it work?</h2>
    
    <div class="space-y-4">
      <div class="flex items-start gap-4">
        <div class="shrink-0 w-10 h-10 rounded-full bg-[#0A6BCE] text-white flex items-center justify-center font-bold">1</div>
        <div>
          <h3 class="font-semibold text-[#0A6BCE] mb-2">Monthly registration</h3>
          <p class="text-[#0f172a]/80">
            You register for the current month. You can participate in a single meeting or commit to several months.
          </p>
        </div>
      </div>
      
      <div class="flex items-start gap-4">
        <div class="shrink-0 w-10 h-10 rounded-full bg-[#0A6BCE] text-white flex items-center justify-center font-bold">2</div>
        <div>
          <h3 class="font-semibold text-[#0A6BCE] mb-2">Receiving information</h3>
          <p class="text-[#0f172a]/80">
            You receive the KMeet link and practical information by email a few days before the meeting.
          </p>
        </div>
      </div>
      
      <div class="flex items-start gap-4">
        <div class="shrink-0 w-10 h-10 rounded-full bg-[#0A6BCE] text-white flex items-center justify-center font-bold">3</div>
        <div>
          <h3 class="font-semibold text-[#0A6BCE] mb-2">Meeting participation</h3>
          <p class="text-[#0f172a]/80">
            On the day, you participate in the 60-minute session via videoconference with the group.
          </p>
        </div>
      </div>
      
      <div class="flex items-start gap-4">
        <div class="shrink-0 w-10 h-10 rounded-full bg-[#0A6BCE] text-white flex items-center justify-center font-bold">4</div>
        <div>
          <h3 class="font-semibold text-[#0A6BCE] mb-2">Access to replay</h3>
          <p class="text-[#0f172a]/80">
            If you cannot attend, you receive access to the replay to review the content at your own pace.
          </p>
        </div>
      </div>
    </div>
  </section>

  <!-- Espace supplémentaire -->
  <div class="py-8"></div>

  <section class="section-card p-8 bg-white space-y-6">
    <h2 class="text-2xl font-semibold text-[#0A6BCE] text-center">Upcoming date</h2>
    
    <div class="max-w-2xl mx-auto space-y-4">
      <div class="border-l-4 border-[#0A6BCE] pl-4 py-2">
        <h3 class="font-semibold text-[#0A6BCE] mb-1">Clarity Meeting - Tuesday, June 2, 2026</h3>
        <p class="text-[#0f172a]/80 text-sm">At 1:30 PM (France/Switzerland/Belgium time)</p>
        <p class="text-[#0f172a]/80 text-sm">Online via KMeet (a browser is enough)</p>
      </div>
      
    </div>
  </section>

  <section class="section-card p-8 bg-white space-y-6">
    <h2 class="text-2xl font-semibold text-[#0A6BCE] text-center">Two formulas to choose from</h2>
    
    <div class="flex flex-col md:flex-row gap-6">
      <!-- Formula 1: Meeting access only -->
      <div class="border-2 border-[#0A6BCE] rounded-lg p-6 flex-1">
        <h3 class="text-xl font-semibold text-[#0A6BCE] text-center mb-4">1. Access to the next meeting only</h3>
        <ul class="space-y-3 text-[#0f172a]/80 mb-6">
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span>Approximately <strong>1h live meeting</strong></span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span>Access to <strong>replay</strong></span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span><strong>Answers to your specific questions</strong></span>
          </li>
          <li class="text-sm text-[#0f172a]/60 italic">
            (during the live session or by email if sent at least 24 hours before)
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">🧭</span>
            <span>Limited to <strong>5 people</strong></span>
          </li>
        </ul>
        <div class="text-center pt-4 border-t border-[#0A6BCE]/20">
          <div class="text-2xl font-bold text-[#0A6BCE] mb-2">100 CHF incl. tax</div>
          <p class="text-sm text-[#0f172a]/60 mb-4">(equivalent in €)</p>
          <button onclick="window.CedricVPayment.redirectToStripe('rdv-clarte', 'en', event, 'unique')" class="btn-primary inline-flex items-center gap-2">
            Register for meeting only
          </button>
        </div>
      </div>

      <!-- Formula 2: Join Fluance Pro -->
      <div class="border-2 border-[#ffce2d] rounded-lg p-6 bg-[#ffce2d]/5 flex-1">
        <div class="text-center mb-4">
          <div class="cta-pill bg-[#0A6BCE] text-white inline-flex mb-2">Recommended</div>
        </div>
        <h3 class="text-xl font-semibold text-[#0A6BCE] text-center mb-4">2. Full access</h3>
        <ul class="space-y-3 text-[#0f172a]/80 mb-6">
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span><strong>All meetings</strong> included, <strong>live</strong> and <strong>replays</strong></span>
          </li>
          <li class="text-sm text-[#0f172a]/60 italic">
            a new one every month
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span><strong>Answers to your specific questions</strong></span>
          </li>
          <li class="text-sm text-[#0f172a]/60 italic">
            (during the live session or by email if sent at least 24 hours before)
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span>Access to the <strong>private Telegram community</strong></span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span><strong>130 CHF discount</strong> on the 1:1 Growth formula</span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0A6BCE] font-bold">✔</span>
            <span><strong>Free training</strong>: the basics of a fluid activity, lessons from 13 years of entrepreneurship (value 500 CHF / €)</span>
          </li>
        </ul>
        <div class="text-center pt-4 border-t border-[#0A6BCE]/20">
          <div class="text-2xl font-bold text-[#0A6BCE] mb-2">69 CHF incl. tax</div>
          <p class="text-sm text-[#0f172a]/60 mb-1">(equivalent in €)</p>
          <p class="text-sm text-[#0f172a]/80 mb-4"><strong>per month, easily cancellable at any time.</strong></p>
          <p class="text-xs text-[#0f172a]/60 mb-4">
            (via client area or email to support@fluance.io)
          </p>
          <p class="text-sm text-[#0A6BCE] font-semibold mb-4">🥳 Launch price, will increase later.</p>
          <button onclick="window.CedricVPayment.redirectToStripe('rdv-clarte', 'en', event, 'abonnement')" class="btn-primary inline-flex items-center gap-2">
            Get full access
          </button>
        </div>
      </div>
    </div>
  </section>

  <section class="text-center space-y-6">
    <h2 class="text-3xl font-semibold text-[#0A6BCE]">Ready to regain clarity?</h2>
    <p class="text-xl text-[#0f172a]/80">
      Join the next monthly meeting and benefit from regular support to develop your activity with serenity.
    </p>
    
    <div class="pt-6">
      <button onclick="window.CedricVPayment.redirectToStripe('rdv-clarte', 'en', event, 'unique')" class="btn-primary inline-flex items-center gap-2">
        Register for the next Clarity Meeting
      </button>
    </div>
    
    <p class="text-[#0f172a]/80">
      Do you have questions?<br>
      <a href="{{ '/en/contact/' | relativeUrl }}" class="text-[#0A6BCE] hover:underline">Contact me</a>
    </p>
  </section>

  <!-- Testimonials -->
  <section class="section-card p-8 bg-white">
    <h2 class="text-2xl font-semibold text-[#0A6BCE] text-center mb-6">What participants say</h2>
    
    <div class="max-w-3xl mx-auto space-y-6">
      <!-- Nathalie Varlet testimonial -->
      <div class="border-l-4 border-[#0A6BCE] pl-6 py-4 bg-[#0A6BCE]/5 rounded-r-lg">
        <p class="text-[#0f172a]/80 text-lg italic mb-4">
          "This is exactly what I need. Every month, it allows me to <strong>take a step back from my activities</strong> and have a <strong>clear and coherent path aligned with who I am</strong> to move forward. Plus, it's <strong>affordable</strong> !"
        </p>
        <p class="text-[#0A6BCE] font-semibold">— Nathalie Varlet</p>
      </div>
    </div>
  </section>
</section>

